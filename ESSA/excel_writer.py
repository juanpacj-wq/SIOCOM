...
class ExcelWriter:
    ...
    def escribir_datos(self, datos: Dict[str, Any]):
        ...
        # 4. Hoja de datos del comercializador
        if 'datos_comercializador' in datos and datos['datos_comercializador']:
            self._crear_hoja_comercializador(datos['datos_comercializador'])
        
        # 5. Hoja de periodo y fechas
        if 'periodo_facturacion' in datos and datos['periodo_facturacion']:
            self._crear_hoja_periodo(datos['periodo_facturacion'])
        
        # 6. Hoja de información de recaudo
        if 'informacion_recaudo' in datos and datos['informacion_recaudo']:
            self._crear_hoja_recaudo(datos['informacion_recaudo'])
        
        # 7. Hoja de todos los datos (tabla completa)
    ...
    def _crear_hoja_resumen(self, datos: Dict[str, Any]):
        ...
        if 'informacion_general' in datos:
            ...
            if 'numero_factura' in info:
                info_clave.append(('Número de Factura', info['numero_factura']))
        ...
        if 'periodo_facturacion' in datos:
            ...
            if 'forma_pago' in periodo:
                info_clave.append(('Forma de Pago', periodo['forma_pago']))
        
        if 'datos_cliente' in datos:
            cliente = datos['datos_cliente']
            if 'codigo_cuenta_niu' in cliente:
                info_clave.append(('Código NIU', cliente['codigo_cuenta_niu']))
    ...
    def _crear_hoja_conceptos(self, conceptos: List[Dict[str, Any]]):
        ...
        if conceptos:
            ...
            fila_inicio = 3
            columnas = ['numero', 'codigo', 'concepto', 'unidad', 'valor_unidad', 'cantidad', 'iva', 'valor_mes']
            nombres_columnas = ['No.', 'Código', 'Concepto', 'Unidad', 'Valor Unitario', 'Cantidad', 'IVA', 'Valor Mes']
            
            for col_idx, (col_key, col_name) in enumerate(zip(columnas, nombres_columnas), 1):
            ...
            for row_idx, concepto in enumerate(conceptos, start=fila_inicio + 1):
                for col_idx, col_key in enumerate(columnas, 1):
                    ...
                    # Color alternado
                    if row_idx % 2 == 0:
                        celda.fill = self.relleno_alternado
            
            # Agregar fila de total
            fila_total = fila_inicio + len(conceptos) + 1
            ws[f'A{fila_total}'] = 'TOTAL'
            ws[f'A{fila_total}'].font = self.estilo_encabezado
            
            # Calcular y mostrar total
            total = sum(c.get('valor_mes', 0) for c in conceptos)
            celda_total = ws[f'H{fila_total}']
            ...
            # Ajustar anchos de columna
            anchos = [8, 12, 30, 12, 18, 15, 10, 18]
            for col_idx, ancho in enumerate(anchos, 1):
                ws.column_dimensions[chr(64 + col_idx)].width = ancho
    
    def _crear_hoja_comercializador(self, datos: Dict[str, str]):
        """Crear hoja con datos del comercializador"""
        ws = self.workbook.create_sheet("Comercializador")
        
        # Título
        ws.merge_cells('A1:B1')
        ws['A1'] = 'DATOS DEL COMERCIALIZADOR'
        ws['A1'].font = self.estilo_titulo
        ws['A1'].fill = self.relleno_titulo
        ws['A1'].alignment = self.alineacion_centro
    ...
    def _crear_hoja_recaudo(self, recaudo: Dict[str, Any]):
        """Crear hoja con información de recaudo"""
        ws = self.workbook.create_sheet("Información Recaudo")
        
        # Título
        ws.merge_cells('A1:B1')
        ws['A1'] = 'INFORMACIÓN DE RECAUDO'
        ws['A1'].font = self.estilo_titulo
        ws['A1'].fill = self.relleno_titulo
        ws['A1'].alignment = self.alineacion_centro
        ...
        # Datos
        fila = 3
        for campo, valor in recaudo.items():
            ws[f'A{fila}'] = campo.replace('_', ' ').title()
            
            # Manejar listas (como correos)
            if isinstance(valor, list):
                ws[f'B{fila}'] = ', '.join(str(v) for v in valor)
            else:
                ws[f'B{fila}'] = str(valor)
    ...
    def guardar_como_csv(self, datos: Dict[str, Any], archivo_csv: str = "factura_datos.csv"):
        """
